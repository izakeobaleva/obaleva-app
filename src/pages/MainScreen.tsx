...
const LoginForm = ({ onLogin }: any) => {
  ...
  return (
    <>
      ...
      <button onClick={handleSubmit} disabled={loading} className="w-full py-3 rounded-xl bg-[#F4D03F] text-black font-bold">
...
export const MainScreen = () => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      console.log("Sessão:", session?.user?.email || "Nenhum usuário");
    ...
    return (
      <div className="min-h-screen bg-[#0F0B1A] flex items-center justify-center">
        <div className="text-center">
          <div className="w-8 h-8 border-2 border-[#F4D03F] border-t-transparent rounded-full animate-spin mx-auto mb-3" />
      ...
      <div className="min-h-screen bg-[#0F0B1A] p-4">
        <div className="max-w-md mx-auto">
          ...
          <div className="bg-[#1A1528] rounded-xl p-4">
            ...
            />
            <button className="w-full mt-3 py-3 bg-[#F4D03F] text-black font-bold rounded-lg">
              Confirmar corrida
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0F0B1A] to-[#1A1528] flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="bg-[#1A1528] rounded-2xl p-6">
          <button onClick={handleGoogleLogin} className="w-full py-3 rounded-xl border border-white/20 text-white flex items-center justify-center gap-2">
