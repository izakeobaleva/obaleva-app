...
// DiscoverBar (cards roláveis sem título)
const DiscoverBar = () => {
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    {card.type === 'video' && <div className="mt-2 text-[#F4D03F] text-xs">▶️ Assistir</div>}
                    {card.type === 'promo' && <div className="mt-2 inline-block bg-[#F4D03F]/20 text-[#F4D03F] text-xs px-2 py-0.5 rounded-full">Promoção</div>}
                  </div>
                </div>
              </div>
            ))}
...
// Componente de mapa ao vivo (estendido até o topo, com logo sobreposto)
const LiveMap = () => {
  ...
  return (
    <div className="relative h-96 w-full bg-gradient-to-br from-[#2a1a3a] to-[#1a1a2e] flex flex-col items-center justify-center overflow-hidden">
      {/* Elementos de mapa animados */}
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-48 h-48 rounded-full bg-[#F4D03F]/10 animate-pulse" />
        <div className="absolute w-10 h-10 rounded-full bg-[#F4D03F] flex items-center justify-center shadow-lg">
          <MapPin size={20} className="text-black" />
        </div>
      </div>
      
      {/* Localização atual */}
      <div className="absolute bottom-4 left-4 z-10 bg-black/50 backdrop-blur-sm px-3 py-1.5 rounded-full">
      ...
      {/* Logo e slogan sobrepostos ao mapa */}
      <div className="absolute top-8 left-0 right-0 z-10 text-center">
        ...
        <p className="text-white/80 text-sm drop-shadow-lg">Mobilidade premium para sua cidade</p>
      </div>
    </div>
  );
};
...
const LoginScreen = ({ onGoogleLogin, onEmailLogin, loginEmail, setLoginEmail, loginPassword, setLoginPassword, loginLoading }: any) => (
  <div className="bg-[#1A1528]/90 backdrop-blur-sm rounded-2xl p-5 border border-white/10 shadow-xl mt-4 mx-4">
...
const PassengerDashboard = () => (
  <div className="bg-[#1A1528]/90 backdrop-blur-sm rounded-2xl p-4 border border-white/10 mt-4 mx-4">
    <div className="space-y-3">
...
export const MainScreen = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0F0B1A] pb-32">
      {/* MAPA ESTENDIDO ATÉ O TOPO COM LOGO SOBREPOSTO */}
      <LiveMap />

      {/* CONTEÚDO PRINCIPAL (ÁREA DE AÇÃO) */}
      {!user ? (
        <LoginScreen
          onGoogleLogin={async () => {
        ...
        <div className="bg-[#1A1528]/90 backdrop-blur-sm rounded-2xl p-4 border border-white/10 text-center mt-4 mx-4">
