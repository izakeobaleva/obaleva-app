...
export default function RegisterDriver() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      await supabase.from('motoristas').insert({ id: data.user.id, nome, telefone, cnh, cnh_foto: cnhFoto })
  ...
  return (
    <div className="min-h-screen bg-roxo-principal flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md">
        <h1 className="text-2xl font-bold text-roxo-principal text-center mb-6">Cadastro Motorista</h1>
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" required className="w-full p-3 border rounded-lg" value={cnh} onChange={e => setCnh(e.target.value)} placeholder="Número da CNH" />
          </div>
          <UploadFile label="Foto da CNH" onUpload={(url) => setCnhFoto(url)} />
          <div>
