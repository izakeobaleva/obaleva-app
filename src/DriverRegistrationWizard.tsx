...
import { LogOut, User, Phone, CreditCard, MapPin, Car, Key, Shield, Upload, ChevronLeft, CheckCircle, AlertCircle } from 'lucide-react';
...
const DriverRegistrationWizard: React.FC<DriverRegistrationWizardProps> = ({ user, onClose, onSuccess, onLogout }) => {
  ...
  const formatarData = (value: string) => {
    ...
    if (numbers.length >= 3) {
      return numbers.replace(/^(\d{2})(\d{1,2})?(\d{0,4})?/, (match, d, m, a) => {
        let result = d;
        if (m) result += '/' + m;
        if (a) result += '/' + a;
        return result;
  ...
  const handleChange = (setter: any, campo: string, formatFn?: (v: string) => string) => (valor: string) => {
    const formatted = formatFn ? formatFn(valor) : valor;
    setter(formatted);
    const erro = validarCampo(campo, formatted);
    setErros(prev => ({ ...prev, [campo]: erro }));
  };

  const handleDataChange = (valor: string) => {
    const formatted = formatarData(valor);
    setDataNascimento(formatted);
    const erro = validarCampo('dataNascimento', formatted);
    setErros(prev => ({ ...prev, dataNascimento: erro }));
  };
  ...
  const InputField = ({ icon: Icon, placeholder, value, onChange, type = 'text', maxLength, erro, campo, inputMode }: any) => (
    <div className="mb-[16px]">
      <div className={`flex items-center gap-3 px-4 py-[10px] rounded-xl border ${erro ? 'border-red-500/50 bg-red-500/5' : 'border-white/15'} bg-[#1A1528] focus-within:ring-2 focus-within:ring-[#F4D03F] transition-all`}>
        ...
        <input 
          ...
          className="flex-1 bg-transparent text-white outline-none text-[16px] placeholder:text-[#A0A0B0]"
          maxLength={maxLength} 
          inputMode={inputMode || 'text'}
          required
        />
        {value && !erro && <CheckCircle size={14} className="text-green-400 shrink-0" />}
        {value && erro && <AlertCircle size={14} className="text-red-400 shrink-0" />}
      </div>
      {erro && <p className="text-red-400 text-[11px] mt-[4px] ml-1 flex items-center gap-1"><span>⚠</span> {erro}</p>}
    </div>
  );
  ...
  const DateField = ({ icon: Icon, value, onChange, erro, campo }: any) => (
    <div className="mb-[16px]">
      ...
      {erro && <p className="text-red-400 text-[11px] mt-[4px] ml-1 flex items-center gap-1"><span>⚠</span> {erro}</p>}
      <small className="text-[#A0A0B0] text-[10px] ml-1">Use o teclado numérico – a data é formatada automaticamente</small>
    </div>
  );

  const UploadField = ({ label, file, onChange }: any) => (
    <div className="mb-[8px]">
      <label className="text-[#A0A0B0] text-[13px] block mb-[4px]">{label}</label>
      <div 
        className="bg-[#1A1528] rounded-xl border border-dashed border-[#F4D03F]/30 p-3 text-center cursor-pointer hover:bg-white/5 transition active:scale-[0.98]"
        onClick={() => document.getElementById(label.replace(/\s/g, ''))?.click()}
      >
        <Upload size={18} className="text-[#F4D03F] mx-auto mb-[4px]" />
        <p className="text-[#A0A0B0] text-[13px] truncate">{file ? file.name : 'Toque para selecionar'}</p>
        <input 
  ...
  return (
    <div className="fixed inset-0 bg-[#0F0B1A] z-50 flex flex-col" style={{ height: '100dvh', overflow: 'hidden' }}>
      {/* Header fixo */}
      <div className="flex-shrink-0 px-4 py-[8px] border-b border-white/10 bg-[#1A1528] flex items-center justify-between">
      ...
      <div className="flex-shrink-0 px-4 py-1">
        <div className="bg-[#F4D03F]/10 rounded-lg p-1.5 text-center">
          <p className="text-[#F4D03F] text-xs font-bold">
      ...
      <div 
        ...
        style={{ 
          padding: '8px 16px',
          paddingBottom: `${bottomBarHeight + 80}px`,
        }}
      >
        {etapa === 1 && (
          <>
            <InputField icon={User} placeholder="Nome completo *" value={nome} onChange={handleChange(setNome, 'nome')} erro={erros.nome} campo="nome" />
            <InputField icon={Phone} placeholder="WhatsApp *" value={telefone} onChange={handleChange(setTelefone, 'telefone', formatPhone)} maxLength={15} erro={erros.telefone} campo="telefone" inputMode="numeric" />
            <InputField icon={CreditCard} placeholder="CPF *" value={cpf} onChange={handleChange(setCpf, 'cpf', formatCPF)} maxLength={14} erro={erros.cpf} campo="cpf" inputMode="numeric" />
            <DateField icon={Calendar} value={dataNascimento} onChange={handleDataChange} erro={erros.dataNascimento} campo="dataNascimento" />
          </>
        )}
        ...
        {etapa === 4 && (
          <>
            ...
            <InputField icon={Calendar} placeholder="Ano *" value={ano} onChange={handleChange(setAno, 'ano')} maxLength={4} erro={erros.ano} campo="ano" inputMode="numeric" />
            <InputField icon={Car} placeholder="Cor *" value={cor} onChange={handleChange(setCor, 'cor')} erro={erros.cor} campo="cor" />
          </>
        )}

        {/* Mensagem motivacional sem valores monetários */}
        <div className="mt-4 p-3 bg-[#F4D03F]/10 rounded-xl border border-dashed border-[#F4D03F]/30 text-center">
        ...
        <div className="flex gap-2 max-w-md mx-auto">
          {etapa > 1 && (
            <button 
              ...
              className="px-4 py-[10px] rounded-xl border border-white/20 text-white font-bold text-sm flex items-center gap-1 hover:bg-white/5 transition active:scale-[0.97]"
            >
          ...
          {etapa < totalEtapas ? (
            <button 
              ...
              className="flex-1 py-[10px] rounded-xl bg-gradient-to-r from-[#FFD966] to-[#F4D03F] text-[#1E1E2F] font-bold text-sm hover:opacity-90 transition active:scale-[0.97] shadow-lg shadow-[#F4D03F]/20"
            >
              Continuar
            </button>
          ) : (
            <button 
              ...
              disabled={loading}
              className="flex-1 py-[10px] rounded-xl bg-gradient-to-r from-[#22C55E] to-[#16A34A] text-white font-bold text-sm hover:opacity-90 transition active:scale-[0.97] disabled:opacity-50 shadow-lg shadow-green-500/20"
            >
