...
import { User, Edit, CreditCard, History, Star, Bell, Shield, HelpCircle, LogOut, ChevronRight, Award, Truck, Car, DollarSign } from 'lucide-react';
...
const DriverProfile: React.FC<DriverProfileProps> = ({ user, onLogout }) => {
  ...
  const carregarPerfil = async () => {
    ...
    setProfile(userData);
    if (motoristaData?.dados_veiculo) {
  ...
  const carregarEstatisticas = async () => {
    ...
    if (corridas && corridas.length > 0) {
      ...
      setEstatisticas(prev => ({ ...prev, corridas_hoje: 0, faturamento_hoje: 0 }));
    }
  };
      ...
      <div className="max-w-md mx-auto px-4 pb-24 pt-4">
        ...
        <div className="bg-gradient-to-br from-[#1A1528] to-[#2D2342] rounded-2xl p-5 border-2 border-[#F4D03F]/30 shadow-xl">
          ...
          <div className="text-center mt-3">
            ...
            <p className="text-[#A0A0B0] text-xs">{profile?.telefone}</p>
