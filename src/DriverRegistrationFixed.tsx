...
import { LogOut, User, Phone, CreditCard, Calendar, MapPin, Car, Key, Shield } from 'lucide-react';
...
const DriverRegistrationFixed: React.FC<DriverRegistrationFixedProps> = ({ user, onClose, onSuccess, onLogout }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#0F0B1A] z-50 flex flex-col" style={{ height: '100dvh' }}>
      ...
      {/* Conteúdo com scroll - botão no fluxo */}
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5 pb-24">
        ...
        {/* Botão no fluxo - mesmo espaçamento dos campos */}
        <button
          onClick={handleSubmit}
          disabled={loading}
          className="w-full py-3 rounded-xl bg-gradient-to-r from-[#F4D03F] to-[#FFD966] text-black font-bold text-sm hover:opacity-90 transition active:scale-[0.98] disabled:opacity-50 mt-1"
        >
